  ...
  try {
    ...
    if (options.contour) {
      contour = options.autoDownsample
        ...
        : data;
      contour = data.map((d) => interpoliser(d));
...
const interpoliser = (data) => {
  var newData = JSON.parse(JSON.stringify(data));
  for (var y = 1; y < data.z.length - 1; y++) {
    for (var x = 1; x < data.z[y].length - 1; x++) {
      if (data.z[y][x] === null || !isNumeric(data.z[y][x])) {
        data.z[y][x] = 1000;
...
const getNeighbour = (grid, x, y, maxX, maxY) => {
